...
class EnhancedCSTester:
    ...
    def simulate_manual_evaluation(self, scenario: dict, mock_response: str) -> dict:
        ...
        # 3. 检查解决度 - 是否包含关键要点
        matched_points = sum(
            1 for point in key_points if any(kw in mock_response for kw in point.split())
        )
        ...
        if category == "after_sales":
            if "质量问题" in user_msg and not any(
                word in mock_response for word in ["退款", "换货", "承担"]
            ):
    ...
    def run_enhanced_test(self) -> list[dict]:
        ...
        for i, scenario in enumerate(ENHANCED_TEST_SCENARIOS, 1):
            ...
            print(
                f"  {status} {scenario['description']} - 评分: {score:.2f} (目标: {expected:.2f})"
            )
    ...
    def analyze_enhanced_results(self, results: list[dict]) -> dict:
        ...
        return {
            ...
            "pass_rate": len([r for r in results if r["evaluation"]["overall"] >= 0.85])
            / len(results),
    ...
    def print_enhanced_analysis(self, analysis: dict):
        ...
        if analysis["failed_cases"]:
            ...
            for case in analysis["failed_cases"]:
                print(
                    f"  • {case['description']}: {case['actual_score']:.2f}/{case['expected_score']:.2f}"
                )
