...
async def test_endpoint(client: httpx.AsyncClient, method: str, endpoint: str, data: Dict = None) -> Dict[str, Any]:
    """测试单个端点"""
...
async def run_comprehensive_test():
    ...
    test_cases = [
        ...
        {"method": "GET", "endpoint": "/api/competitors/price-comparison", "category": "competitors"},
        ...
        {"method": "GET", "endpoint": "/api/inventory/restock-suggestions", "category": "inventory"},
        ...
        {"method": "POST", "endpoint": "/api/chat", 
         "data": {"message": "我的店铺今天销售怎么样？"}, "category": "chat"},
...
def analyze_results():
    ...
    total_score = sum(
        test_results["area_scores"][area] * weights[area] 
        for area in weights
    )
        ...
        for ep in endpoints:
            ...
            if not ep["success"]:
                ...
                if "dashboard" in ep["endpoint"]:
                    ...
                    report += f"  - 🏪 管理{len(stores)}家门店，总GMV: ¥{data.get('total_gmv', 0)}\n"
