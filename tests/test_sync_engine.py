...
from src.sync.scheduler import SyncScheduler, INTERVAL_MAP
...
class DummySyncer(BaseSyncer):
    ...
    def __init__(self, client=None, db_pool=None):
        ...
        self._full_sync_result = SyncResult(syncer_name="dummy", mode=SyncMode.FULL, success=True, records_synced=10)
        self._incr_sync_result = SyncResult(syncer_name="dummy", mode=SyncMode.INCREMENTAL, success=True, records_synced=5)
...
class TestSyncResult:
    def test_summary_success(self):
        r = SyncResult(syncer_name="test", mode=SyncMode.FULL, success=True, records_synced=42, duration_ms=123)
        assert "✅" in r.summary
...
class TestBaseSyncerSync:
    ...
    async def test_no_full_sync_recorded_triggers_full(self):
        pool = make_pool()
        pool.fetchrow = AsyncMock(return_value={"last_full_sync": None, "last_incremental_sync": None})
        s = DummySyncer(client=make_client(), db_pool=pool)
        result = await s.sync()
        assert s.full_sync_called == 1
    ...
    async def test_stale_full_sync_triggers_full(self):
        ...
        pool.fetchrow = AsyncMock(return_value={"last_full_sync": old_time, "last_incremental_sync": old_time})
        s = DummySyncer(client=make_client(), db_pool=pool)
    ...
    async def test_recent_full_sync_triggers_incremental(self):
        ...
        pool.fetchrow = AsyncMock(return_value={"last_full_sync": recent, "last_incremental_sync": recent})
        s = DummySyncer(client=make_client(), db_pool=pool)
        result = await s.sync()
    ...
    async def test_retry_on_failure(self):
        pool = make_pool()
        s = DummySyncer(client=make_client(), db_pool=pool)
        s._full_sync_result = SyncResult(syncer_name="dummy", mode=SyncMode.FULL, success=False, error="fail")
        result = await s.sync()
...
class TestOrderSyncer:
    ...
    async def test_full_sync(self):
        client = make_client()
        pool = make_pool()
        client.post = AsyncMock(return_value={"data": {"list": [{"orderId": "O1", "platform": "meituan"}], "totalPage": 1}})
...
class TestSyncScheduler:
    ...
    def test_interval_map_completeness(self):
        expected = ["every_15min", "every_30min", "every_1h", "every_2h", "every_4h", "every_6h", "every_12h"]
        for k in expected:
