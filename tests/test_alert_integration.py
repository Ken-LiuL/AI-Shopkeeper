...
class TestAnomalyDetection:
    async def test_detects_anomalies(self, sample_anomalies_found):
        ...
        with patch("src.agents.alert.nodes.call_tool",
                    new_callable=AsyncMock, return_value=sample_anomalies_found):
    ...
    async def test_no_anomalies(self, sample_anomalies_none):
        ...
        with patch("src.agents.alert.nodes.call_tool",
                    new_callable=AsyncMock, return_value=sample_anomalies_none):
            result = await anomaly_detection_node(state)
    ...
    async def test_detection_error(self):
        ...
        with patch("src.agents.alert.nodes.call_tool",
                    new_callable=AsyncMock, side_effect=RuntimeError("prophet crash")):
            result = await anomaly_detection_node(state)
...
class TestShouldAnalyze:
    def test_no_anomalies_returns_end(self, sample_anomalies_none):
        from src.agents.alert.graph import build_alert_graph
...
class TestRootCause:
    async def test_analyzes_non_info_anomalies(self, sample_anomalies_found, sample_root_cause):
        ...
        with patch("src.agents.alert.nodes.call_tool",
                    new_callable=AsyncMock, return_value=sample_root_cause):
    ...
    async def test_skips_info_severity(self, sample_root_cause):
        ...
        state: AlertState = {
            "anomalies": {
                "anomalies": [
                    {"anomaly_id": "A003", "product_id": "P300", "anomaly_type": "overstock",
                     "severity": "info", "description": "库存略多"},
                ],
            },
            "root_causes": [],
            "errors": [],
        ...
        with patch("src.agents.alert.nodes.call_tool",
                    new_callable=AsyncMock, return_value=sample_root_cause) as mock_ct:
            result = await root_cause_node(state)
    ...
    async def test_partial_failure(self, sample_anomalies_found, sample_root_cause):
        ...
        with patch("src.agents.alert.nodes.call_tool", new_callable=AsyncMock, side_effect=_side_effect):
            result = await root_cause_node(state)
...
class TestAction:
    async def test_generates_actions(self, sample_root_cause, sample_action):
        ...
        with patch("src.agents.alert.nodes.call_tool",
                    new_callable=AsyncMock, return_value=sample_action):
    ...
    async def test_action_error(self, sample_root_cause):
        ...
        with patch("src.agents.alert.nodes.call_tool",
                    new_callable=AsyncMock, side_effect=RuntimeError("llm err")):
            result = await action_node(state)
...
class TestEndToEnd:
    ...
    async def test_full_anomaly_flow(self, sample_anomalies_found, sample_root_cause, sample_action):
