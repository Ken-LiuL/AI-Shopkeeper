        ...
        if store_raw_rows:
            ...
            for row in store_raw_rows:
                ...
                if isinstance(raw_data, dict):
                    ...
                    record_traffic = _extract_metric(raw_data, "expose_cnt") or _extract_metric(raw_data, "pv_cnt")
            ...
            if valid_records > 0:
                ...
                if metrics["traffic"] > 0:
                    metrics["conversion_rate"] = round(metrics["customers"] / metrics["traffic"] * 100, 2)
...
async def get_store_summary(poi_id: int) -> APIResponse[Dict]:
    ...
    if poi_id not in STORE_POI_IDS:
        raise HTTPException(
            status_code=404,
            detail=f"门店POI {poi_id}不在管理范围内。当前管理门店: {STORE_POI_IDS}"
        )
    ...
    try:
        ...
        # 获取昨日对比
        yesterday_metrics = await _get_store_metrics_from_raw(poi_id, current_date - timedelta(days=1))
        ...
        if yesterday_metrics["gmv"] > 0:
            growth_metrics["gmv_growth"] = (today_metrics["gmv"] - yesterday_metrics["gmv"]) / yesterday_metrics["gmv"] * 100
        
        if yesterday_metrics["orders"] > 0:
            growth_metrics["orders_growth"] = (today_metrics["orders"] - yesterday_metrics["orders"]) / yesterday_metrics["orders"] * 100
        
        if yesterday_metrics["avg_order_value"] > 0:
            growth_metrics["aov_growth"] = (today_metrics["avg_order_value"] - yesterday_metrics["avg_order_value"]) / yesterday_metrics["avg_order_value"] * 100
...
async def _analyze_store_characteristics(poi_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        if category_mix:
            ...
            characteristics["product_mix"] = f"主营{top_category}，占销量{category_mix[0]['total_sold']}%"
    ...
    try:
        ...
        result = {
            ...
            "summary": {
                ...
                "avg_store_gmv": sum(s["total_gmv"] for s in comparison_data.values()) / len(comparison_data),
                "top_performer": stores_ranking[0]["name"] if stores_ranking else "",
                "most_consistent": max(comparison_data.values(), key=lambda x: x["consistency_score"], default={}).get("name", "")
            }
        }
