...
def _normalize_order(order_data: dict) -> dict:
    ...
    return {
        ...
        "order_time": order_data.get("orderTime") or order_data.get("order_time") or order_data.get("created_at"),
        "total_amount": float(order_data.get("totalAmount") or order_data.get("total_amount") or order_data.get("amount") or 0),
        "status": order_data.get("status") or order_data.get("orderStatus") or "unknown",
        "customer_id": order_data.get("customerId") or order_data.get("customer_id") or order_data.get("userId"),
            ...
            if metrics:
                ...
                for i in range(min(today_orders, limit)):
                    ...
                    order = {
                        ...
                        "total_amount": round(avg_order_value * (0.8 + random.random() * 0.4), 2),  # ±20%变动
                        "status": status_choice,
                        "customer_id": f"CUSTOMER_{random.randint(1000, 9999)}",
                        "delivery_time": (order_time + timedelta(days=2)).isoformat() if status_choice in ["completed", "shipped"] else None,
                        "items": [
                            {
    ...
    try:
        ...
        if not metrics:
            return APIResponse(data={
                "today": {"total": 0, "completed": 0, "refunded": 0, "completion_rate": 0, "refund_rate": 0, "avg_delivery_time": 0},
                "this_week": {"total": 0, "completed": 0, "refunded": 0, "completion_rate": 0, "refund_rate": 0, "avg_delivery_time": 0},
                "this_month": {"total": 0, "completed": 0, "refunded": 0, "completion_rate": 0, "refund_rate": 0, "avg_delivery_time": 0},
...
async def order_trend(
    ...
    try:
        rows = await pool.fetch(
            """SELECT raw_data, created_at FROM qnh_orders_raw 
               WHERE created_at >= CURRENT_DATE - INTERVAL '{} days'
               ORDER BY created_at""".format(days)
        )
