            ...
            "properties": {
                "suggestions": {
                    ...
                    "items": {
                        ...
                        "required": ["product_id", "suggested_qty", "urgency", "timing", "reasoning"]
                    }
                }
            },
    ...
    try:
        ...
        if analysis_data:
            ...
            for suggestion in suggestions:
                ai_suggestion = next((s for s in ai_suggestions if s["product_id"] == suggestion.product_id), None)
                if ai_suggestion:
                    suggestion.suggested_restock_qty = max(ai_suggestion["suggested_qty"], suggestion.suggested_restock_qty)
...
async def get_inventory_overview() -> APIResponse[Dict]:
    ...
    try:
        ...
        result = {
            ...
            "total_stock": int(overview["total_stock"]) if overview["total_stock"] else 0,
            "low_stock_count": int(overview["low_stock_count"]) if overview["low_stock_count"] else 0,
            "out_of_stock_count": int(overview["out_of_stock_count"]) if overview["out_of_stock_count"] else 0,
            "avg_stock": round(float(overview["avg_stock"]), 2) if overview["avg_stock"] else 0,
            "restock_alerts": {
                "high_priority": high_priority,
                "medium_priority": medium_priority,
                "total": len(restock_result.data)
            },
            "category_breakdown": [
                {
                    ...
                    "avg_stock": round(float(row["avg_stock"]), 2)
                }
    ...
    try:
        ...
        turnover_data = await pool.fetch("""
            ...
            LEFT JOIN orders o ON oi.order_id = o.order_id 
                AND o.order_time >= CURRENT_DATE - INTERVAL '{} days'
            WHERE p.status = 'active'
        ...
        for row in turnover_data:
            result.append({
                ...
                "turnover_category": (
                    "快速" if row["annual_turnover_rate"] > 12 else
                    "正常" if row["annual_turnover_rate"] > 4 else
                    "缓慢"
                )
            })
        ...
        return APIResponse(
            success=False,
            message=f"Failed to calculate turnover: {str(e)}",
            data=[]
        )