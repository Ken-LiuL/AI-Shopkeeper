...
async def list_stores(
    ...
    pool = pg.get_pool()
    total = await pool.fetchval("SELECT COUNT(*) FROM competitor_stores")
...
async def hot_keywords(
    ...
    pool = pg.get_pool()
    rows = await pool.fetch(
        """SELECT keyword, search_volume, heat_score, trend, updated_at
           FROM market_keywords ORDER BY heat_score DESC LIMIT $1""",
        limit,
    )
...
async def price_comparison(
    ...
    idx = 1
    if product_id:
        conditions.append(f"p.product_id = ${idx}")
    ...
    params.append(limit)
    rows = await pool.fetch(
        f"""SELECT p.product_id, p.name, p.retail_price AS our_price,
                   cp.name AS competitor_name, cp.price AS competitor_price,
                   cs.store_name AS competitor_store,
                   ROUND((p.retail_price - cp.price) / NULLIF(cp.price, 0) * 100, 2) AS price_diff_pct
            FROM products p
            JOIN competitor_products cp ON cp.category = p.category AND cp.name ILIKE '%' || p.name || '%'
