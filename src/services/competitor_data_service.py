...
class CompetitorDataService:
    ...
    async def get_enhanced_competitor_prices(
        ...
        """获取增强版竞品价格数据，优先使用真实数据源"""

        cache_key = f"{product_id}_{category}"
        ...
        # 4. 最后才使用标记清楚的演示数据
        logger.warning(f"使用演示数据（已标注）: {product_name}")
        return await self._generate_demo_competitors(product_name, our_price, category)
