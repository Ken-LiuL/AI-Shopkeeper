...
class AdaptiveThresholds:
    ...
    async def update_from_feedback(
        ...
        false_positive_rate = false_positive_count / (
            false_positive_count + true_negative_count + 0.001
        )
        false_negative_rate = false_negative_count / (
            false_negative_count + true_positive_count + 0.001
        )
        ...
        logger.info(
            f"Threshold {threshold_name} updated: {old_value:.2f} → {new_value:.2f} ({reason})"
        )
