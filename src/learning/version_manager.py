...
class ParameterVersionManager:
    ...
    def __init__(self, pool: Any = None):
        ...
        self._active_versions: dict[ParameterType, str | None] = {
    ...
    async def create_version(
        ...
        version_id = (
            f"{param_type.value}_{datetime.now().strftime('%Y%m%d%H%M%S')}_{uuid.uuid4().hex[:6]}"
        )
    ...
    async def rollback(self, param_type: ParameterType, steps: int = 1) -> ParameterVersion | None:
        """
        回滚到之前的版本
    ...
    def get_active_version(self, param_type: ParameterType) -> ParameterVersion | None:
        """获取当前激活的版本"""
    ...
    async def update_performance_score(
        ...
        """更新版本的表现评分"""
        for _param_type, versions in self._versions.items():
            for v in versions:
                if v.version_id == version_id:
    ...
    def get_best_performing_version(
        ...
        """获取表现最好的版本"""
        versions = [
            v for v in self._versions.get(param_type, []) if v.performance_score is not None
        ]
        if not versions:
            return None
