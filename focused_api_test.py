...
def test_endpoint(client: httpx.Client, method: str, path: str, payload: Dict[str, Any] = None) -> Dict[str, Any]:
...
def main():
    ...
    try:
        ...
        if result.get("status_code") == 200:
            ...
            if isinstance(gmv, (int, float)) or (isinstance(gmv, str) and gmv.replace(".", "").replace("-", "").isdigit()):
            ...
            if products > 0:
                ...
                print(f"❌ Zero products - possible data sync issue")
        ...
        if result.get("status_code") == 200:
            ...
            if comparisons:
                ...
                if meaningful_diffs > 0:
                    print(f"✅ Found {meaningful_diffs}/{len(comparisons)} meaningful price differences")
                else:
                    print(f"⚠️  All {len(comparisons)} price differences are < 0.1% - might indicate comparison issues")
            else:
        ...
        if result.get("status_code") == 200:
            ...
            if runs:
                ...
                if latest_run and "run_id" in latest_run:
                    detail_result = test_endpoint(client, "GET", f"/api/selection/runs/{latest_run['run_id']}")
        ...
        for endpoint in chinese_endpoints:
            ...
            if result.get("status_code") == 200:
                ...
                if "中" in response_text or "医" in response_text or any(ord(c) > 127 for c in response_text[:1000]):
